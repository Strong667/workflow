...
Страница «Сотрудники» — единый список людей компании: и рядовых, и менеджеров, и администраторов. Создание сотрудника сразу заводит ему вход: email из карточки становится логином, пароль и роль задаются в той же форме. Отдельного раздела для аккаунтов нет — роль и пароль меняются там же, где остальные данные, а удаление сотрудника убирает и его учётную запись.

Технически это по-прежнему две таблицы из ТЗ — `employees` и `users`, связанные полем `employees.user_id`; форма просто работает с ними как с одной записью.

Что даёт связь рядовому сотруднику:
...
Ограничения ролей: менеджер не выдаёт роль администратора и не трогает карточки администраторов; нельзя удалить собственную учётную запись и последнего администратора.
...
| GET/POST/PUT/DELETE | `/api/employees` | CRUD сотрудников вместе с их аккаунтами (поиск, фильтры, сортировка, пагинация) |
...
- `admin` — полный доступ, включая карточки и аккаунты администраторов;
- `manager` — то же, кроме администраторов: не может ни выдать роль `admin`, ни изменить или удалить их карточки;
